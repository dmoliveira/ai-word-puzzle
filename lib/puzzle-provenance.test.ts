...
for (const expected of fingerprints) {
  test(`generator v3 provenance remains exact: ${expected.name}`, () => {
    ...
    const run = input.boardView === "quest"
      ? buildQuestPuzzleRun(input, Date.parse("2030-06-15T12:00:00.000Z"), { generatorVersion: 3 })
      : buildPuzzleRun(input, Date.parse("2030-06-15T12:00:00.000Z"), { generatorVersion: 3 });
...
test("v3 Quest provenance includes the complete materialized display grid", () => {
  ...
  const run = buildQuestPuzzleRun(input, Date.now(), { generatorVersion: 3 });
...
test("Quest v4 freezes independent q4 identity and p1 provenance", () => {
  const run = buildQuestPuzzleRun({ mode: "custom", seed: "trace-myth", topics: ["myth"], puzzleSize: 6, boardView: "quest" }, Date.parse("2030-06-15T12:00:00.000Z"));
  ...
  assert.equal(hasVerifiedPuzzleProvenance(run), true);
  const presentation = buildQuestPuzzleRun({ ...run.options, style: "classic", timerEnabled: false, learningMode: true }, Date.parse("2040-01-01T00:00:00.000Z"));
